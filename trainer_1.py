...
class Trainer(object):
        ...
        if userK > 0:
            with torch.no_grad():
                ...
                for batch_users in utils.minibatch(users, batch_size=u_batch_size):
                    ...
                    sorted_values, sorted_indices = torch.sort(rating_pred_t, descending=True)
                    ...
                    for batch_i in range(partial_batch_pred_list_in.shape[0]):
                        ...
                        if str(uid) in items.keys():
                            if len(items[str(uid)])<topk:
                                ...
                                user_pred = partial_batch_pred_list_out[batch_i]
                                for eachpred in user_pred:
                                    if (eachpred in items[str(uid)]):
                                        distill_user_row.append(uid)
                                        distill_item_col.append(int(eachpred))
    ...
    def save_model(self, model):
        torch.save(model.state_dict(), os.path.join(self.model_path, self.model_name +'_'+ self.TIME + '.pth'))
